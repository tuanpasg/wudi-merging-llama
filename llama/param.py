...
class param:
    ...
    def add_new_param(self, new_params):
        device = list(self.values())[0].device
        new_params = {param_name:param.to(device) for param_name, param in new_params.items()}

        new_params_add = {}
        for key in new_params.keys():
            if key not in self.param_dict.keys():
                new_params_add[key] = new_params[key]
        self.param_dict.update(new_params_add)
