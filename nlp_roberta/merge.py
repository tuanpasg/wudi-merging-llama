...
from decompose import get_common_task_vector
class MergingMethod:
    ...
    @utils.args_inspector
    # @torch.inference_mode()
    def task_arithmetic_decompose(
        self,
        base_model: nn.Module,
        models_to_merge: param,
        scaling: float = 1.0,
        merging_single_param: bool = False,
        l1_coef: float = 0.0
    ):
        from copy import deepcopy
        from lamassu import get_merge_vector
        base_model_param = deepcopy(param(base_model))
        merge_model = deepcopy(base_model)
        keys = models_to_merge[0].param_dict.keys()
        for key in tqdm.tqdm(keys):
            merging_vector = get_merge_vector(deepcopy(torch.stack([model.param_dict[key] - base_model_param.param_dict[key] for model in models_to_merge])).detach(),iter_num=300)
            merge_model.param_dict[key] = merge_model.param_dict[key] + merging_vector.cuda()
            merge_model.param_dict[key] = merge_model.param_dict[key].detach()            
        return merge_model
    ...
    #     keys = models_to_merge[0].param_dict.keys()
    #     for key in tqdm.tqdm(keys):
    #         merging_vector = lamassu(deepcopy(torch.stack([model.param_dict[key] - base_model_param.param_dict[key] for model in models_to_merge])).detach(),iter_num=300)
    #         base_model.param_dict[key] += model.param_dict[key] + merging_vector.cuda()
    #         base_model.param_dict[key] = base_model.param_dict[key].detach()
